      ...
      <article className="packet-document packet-document--client">
        ...
        <header className="packet-cover">
          ...
          <section className="packet-dashboard" aria-labelledby="packet-executive-dashboard-title">
            ...
            <section className="packet-readiness-factors" aria-label="Readiness calculation">
              <div><p className="packet-dashboard-label">How readiness is calculated</p><span>{dashboard.readiness.completed} of {dashboard.readiness.total} packet factors pass</span></div>
        ...
        <section className="packet-section" aria-label="Current Status">
          ...
          <div className="packet-current-status">
            <strong>Recorded workflow status: {packetModel.current_status.label}</strong>
            <span>Authoritative readiness state: {dashboard.permit_status} · case record updated {packetModel.case_summary.updated_at_label}</span>
          </div>
        </section>
        ...
        <PacketSection id="agency_follow_up_kit">
          {packetModel.action_kit ? <div className="reviewer-block packet-follow-up-kit"><div><p className="packet-dashboard-label">Concise follow-up email</p><h3>{packetModel.action_kit.email_subject}</h3><p><strong>To:</strong> {packetModel.action_kit.recipient_role}</p><p>{packetModel.action_kit.message_body}</p></div><small>Supported by {packetModel.action_kit.citation_references.join(", ")}</small><div><h4>Call script</h4><ol>{packetModel.action_kit.call_checklist.map(item=><li key={item}>{item}</li>)}</ol></div><div><h4>Requested confirmations</h4><ul>{packetModel.action_kit.requested_confirmations.map(item=><li key={item}>{item}</li>)}</ul></div><div><h4>Documents to have ready</h4>{packetModel.action_kit.documents_ready.length ? <ul>{packetModel.action_kit.documents_ready.map(item=><li key={item}>{item}</li>)}</ul> : <p>Use only the cited packet sources listed above.</p>}</div><aside><h4>Escalation summary</h4><p>{packetModel.action_kit.escalation_trigger}</p><h4>Next contact recommendation</h4><p>{packetModel.action_kit.recipient_role}</p></aside>{packetModel.action_kit.follow_up_date&&<p><strong>Review date:</strong> {packetModel.action_kit.follow_up_date}</p>}</div> : <p className="packet-empty packet-empty--client">No reviewer-approved findings support an Agency Follow-Up Kit for this edition.</p>}
        </PacketSection>
