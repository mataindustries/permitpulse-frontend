  ...
  return (
    <section className="packet-preview" aria-labelledby="packet-preview-title">
      ...
      <article className="packet-document packet-document--client">
        {packetModel.demonstration_notice && <div className="packet-demo-banner" role="note">{packetModel.demonstration_notice}</div>}
        <header className="packet-cover">
          ...
          <section className="packet-dashboard" aria-labelledby="packet-executive-dashboard-title">
            ...
            {packetModel.action_kit&&<dl className="packet-client-meta"><div><dt>Record confirms</dt><dd>{packetModel.action_kit.confirmed_record}</dd></div><div><dt>Record does not confirm</dt><dd>{packetModel.action_kit.unconfirmed_record}</dd></div><div><dt>Primary unresolved issue</dt><dd>{packetModel.action_kit.primary_blocker}</dd></div><div><dt>Why this next step</dt><dd>{packetModel.action_kit.why_appropriate}</dd></div><div><dt>Packet evidence readiness</dt><dd>{packetModel.action_kit.evidence_readiness}</dd></div><div><dt>Jurisdiction position</dt><dd>{packetModel.action_kit.review_readiness}</dd></div></dl>}
            ...
            <section className="packet-readiness-factors" aria-label="Readiness calculation">
              <div><p className="packet-dashboard-label">Packet readiness checks</p><span>{dashboard.readiness.completed} of {dashboard.readiness.total} complete</span></div>
            ...
            <div className="packet-dashboard-grid">
              <section className="packet-dashboard-panel">
                <p className="packet-dashboard-label">Packet-readiness conditions</p>
                {dashboard.blockers.length > 0 ? (
                  ...
                  <p className="packet-dashboard-clear"><strong>No packet-readiness conditions remain.</strong><span>The packet record is complete enough for delivery review. Open findings do not indicate jurisdiction resolution.</span></p>
                )}
        ...
        <section className="packet-section" aria-label="Current Status">
          <p className="packet-section-intro">Recorded case workflow at generation time. This field does not represent packet readiness or a final jurisdiction disposition.</p>
          <div className="packet-current-status">
            <strong>Case workflow status: {packetModel.current_status.label}</strong>
            <span>Investigation state: {dashboard.permit_status} · packet readiness: {dashboard.readiness.completed} of {dashboard.readiness.total} checks complete · jurisdiction resolution not established</span>
          </div>
        </section>
        ...
        <PacketSection id="evidence_matrix">
          ...
          <div className="reviewer-table">{packetModel.evidence_summaries.map(item=><article key={item.id}><strong>{item.reference} · {item.title}</strong><span>{item.evidence_type_label} · {item.source.date_label} · {item.verification_label} · {item.contributor_label ?? "Contributor not recorded"}</span><p>{item.summary}</p></article>)}</div>
        </PacketSection>
        ...
        <PacketSection id="findings">
          ...
          {(packetModel.agency_dependencies ?? []).length > 0 && <div className="packet-dependency-map" aria-label="Agency dependency map">
            <div className="packet-dependency-map__heading"><span>Agency dependency map</span><p>Approved findings translated into review sequence.</p></div>
            {(packetModel.agency_dependencies ?? []).map((dependency) => <article key={dependency.id}>
              <div><span>Discipline</span><strong>{dependency.discipline}</strong></div><i aria-hidden="true">↓</i>
              <div><span>Blocking issue</span><strong>{dependency.blocking_issue}</strong></div><i aria-hidden="true">↓</i>
              <div><span>Dependent review</span><strong>{dependency.dependent_review}</strong></div><i aria-hidden="true">↓</i>
              <div><span>Recommended next step</span><strong>{dependency.recommended_next_step}</strong><small>{dependency.citation_references.join(", ")}</small></div>
            </article>)}
          </div>}
        </PacketSection>
        ...
        <PacketSection id="agency_follow_up_kit">
          {packetModel.action_kit ? <div className="reviewer-block packet-follow-up-kit"><div><p className="packet-dashboard-label">Agency follow-up email</p><h3>{packetModel.action_kit.email_subject}</h3><p><strong>Recommended contact:</strong> {packetModel.action_kit.recipient_role}</p><p>{packetModel.action_kit.message_body}</p></div><small>Supported by {packetModel.action_kit.citation_references.join(", ")}</small><div><h4>Call script</h4><ol>{packetModel.action_kit.call_checklist.map(item=><li key={item}>{item}</li>)}</ol></div><div><h4>Requested confirmations</h4><ul>{packetModel.action_kit.requested_confirmations.map(item=><li key={item}>{item}</li>)}</ul></div><div><h4>Documents to have ready</h4>{packetModel.action_kit.documents_ready.length ? <ul>{packetModel.action_kit.documents_ready.map(item=><li key={item}>{item}</li>)}</ul> : <p>Use only the cited packet sources listed above.</p>}</div><aside><h4>Escalation summary</h4><p>{packetModel.action_kit.escalation_trigger}</p><h4>Recommended next contact</h4><p>{packetModel.action_kit.recipient_role}</p></aside>{packetModel.action_kit.follow_up_date&&<p><strong>Review date:</strong> {packetModel.action_kit.follow_up_date}</p>}</div> : <p className="packet-empty packet-empty--client">No reviewer-approved findings support an Agency Follow-Up Kit for this edition.</p>}
        </PacketSection>
              ...
              {packetModel.supporting_sources.map((source, index) => {
                ...
                return (
                  <div className="packet-source-list__row" role="row" key={source.id}>
                    <div><span>{String(index + 1).padStart(2, "0")}</span><strong>{source.title}</strong><small>{sourceLabel} · {sourceDate} · {source.contributor_label ?? "Contributor not recorded"}</small></div>
